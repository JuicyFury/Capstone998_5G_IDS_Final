...
main(int argc, char* argv[])
{
    // Basic scenario
    uint16_t gNbNum = 1;
    uint16_t ueTotal = 30;  //if enableAttack is true the last 5 nodes will work as attacker
    ...
    Time AttackpacketInterval = Seconds(0.0001);
    ...
    //UE will be psotioned randomo in 500x 500 areea
    // UEs with random initial positions and mobility
	...
	uePosAlloc->SetAttribute("Z", DoubleValue(0)); // Simple double value
...
// Choose mobility model:
	...
	if(enableAttack)
	   dis = std::uniform_int_distribution<>(0, ueTotal - 6);//excelude the last 5
	else
	  dis = std::uniform_int_distribution<>(0, ueTotal - 1);
	...
	//lets deal with attackers,, if attack is eanbled, i will pick last 5 as attacker
	std::vector<uint32_t> ddosAttackers;
	if(enableAttack)
	   {
	     for(int i=ueTotal-5; i<ueTotal ; i++)
	     {
	       ddosAttackers.push_back(i); //push last 5
	     }
	   }
	   //Now create ddos clients
	   if(enableAttack){
	   for (uint32_t clientIndex : ddosAttackers) {
	    // Select a random server
	    auto it = serverUEs.begin();
	    std::advance(it, dis(gen) % serverUEs.size());
	    uint32_t serverIndex = *it;
	    ...
	    UdpClientHelper udpClient(serverAddress, serverPort);
	    udpClient.SetAttribute("MaxPackets", UintegerValue(0xFFFFFFFF));
	    udpClient.SetAttribute("Interval", TimeValue(AttackpacketInterval));
	    udpClient.SetAttribute("PacketSize", UintegerValue(1024));
	    
	    ApplicationContainer ddosApp = udpClient.Install(ueNodes.Get(clientIndex));
	    ddosApp.Start(appStart);
	    ddosApp.Stop(simTime);
	    
	    std::cout << "DDoS UE" << clientIndex << " -> Server UE" << serverIndex 
		 << " (" << serverAddress << ":" << serverPort << ")" << std::endl;
	}
 ...
 //Let add animation to simulation
	...
	AnimationInterface anim("25-nodes-animation-static-ddos.xml");
	...
	if(enableAttack){
	for (uint32_t i = ueNodes.GetN() - 5; i < ueNodes.GetN(); ++i) {
	    anim.UpdateNodeDescription(ueNodes.Get(i), "ddos");
	    anim.UpdateNodeColor(ueNodes.Get(i), 255, 0, 0);  // yellow
	}
	}
